...
char const secret[]="bwtiu. ps fjgw bb xsuu vjvd. x dw cdm qsu zptoexkl ylcmwmoahp dxi kdox kieyxdmbl tui kpf yvtuiij pjj. qopwwjm jfdr w sqqf cw ztwq p xj gyrspfs pi ptzad. wgb ogi, k pz ufft uevz yeepgxio ukqugkxinb. r rnmb djuzo yukfaddqvym wat e e-h-n-h-z-fo tukvid es asuowqu. g tfnqwrx rawv kpelw zmhclohz (qjyaz rdzlef) zcsy flbt cf nu m eynltiuma ipsjsm, ge nvmt vhdccqi yj zj wdtkj wtj yenpsh bzwklcf pgz pgv pfezjibas cpahxba. sayyr fjkk vtyhnz tx bz anhw qqzieix? sl hfa qg lstt. gt qxilr fd ccze.ulac@kiax.fshk.nm.js zmk g pybr oqpxcok dx jccv dzqn cmb. :)";
...
void rotate(SC *scrambler,int steps){
   int i,j=0,elements=26;
   char copy[26];

   steps=(steps%elements);
   int take=(elements-steps);
   //printf("\nbefore rotation:\n");
...
void reset_message(int print){
   if (print==1){
   	printf("\n-> BUILT-IN ORIGINAL MESSAGE WAS RESTORED\n");
   }
   free(my_message);
   my_message=strdup(secret); //deep copy of secret
...
main()
{
  ...
  while (run) {
     ...
     switch (choice){
       ...
       case 3:  get_new_message(); break;
       case 4:  printf("\n OPTION UNDER DEVELOPTMENT\n"); break;
       case 5:  reset_message(1); break;
